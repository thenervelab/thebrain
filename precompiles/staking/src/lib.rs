...
// Inc. Pallet-evm-precompile-staking package used in Hippius Network in terms of GPLv3.
	...
	Runtime::AccountId: From<WrappedAccountId32>,
{
	...
	#[precompile::public("getValidators()")]
	#[precompile::public("get_validators()")]
	#[precompile::view]
	fn get_validators(handle: &mut impl PrecompileHandle) -> EvmResult<Vec<Address>> {
		...
		let validators = pallet_staking::Validators::<Runtime>::iter()
			.map(|(validator_account, _)| {
				...
				let account_bytes = validator_account.encode();
