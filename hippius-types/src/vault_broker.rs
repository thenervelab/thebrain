...
impl BrokerScope {
    pub fn validate(&self) -> Result<()> {
        if self.vm_id.is_empty() {
            return Err(schema("scope.vm_id must be non-empty".into()));
        }
        if self.luks_path.is_empty() {
            return Err(schema("scope.luks_path must be non-empty".into()));
        }
        if self.userdata_path.is_empty() {
            return Err(schema("scope.userdata_path must be non-empty".into()));
        }
        Ok(())
    }
