...
impl Flavor {
    ...
    /// Reverse of [`Self::vcpus`]: the catalogue flavor whose vCPU
    /// count equals `vcpus`, or `None` if no flavor has that count.
    /// vCPU counts are unique across the catalogue (1/2/4/8/16/32), so
    /// this is a well-defined inverse. Used by the miner-agent's
    /// tenant-preflight capacity gate to recover a flavor's `memory_mb`
    /// from the preflight order's `cpu_count` (the preflight order
    /// carries the measured vCPU count but not the RAM figure).
    pub const fn from_vcpus(vcpus: u8) -> Option<Self> {
        match vcpus {
            1 => Some(Self::Small),
            2 => Some(Self::Medium),
            4 => Some(Self::Large),
            8 => Some(Self::Xlarge),
            16 => Some(Self::X2large),
            32 => Some(Self::X4large),
            _ => None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn from_vcpus_is_the_inverse_of_vcpus() {
        // Every catalogue flavor round-trips through vcpus()/from_vcpus,
        // so the miner-agent preflight gate can recover a flavor's RAM
        // from the measured vCPU count alone.
        for &f in Flavor::all() {
            assert_eq!(Flavor::from_vcpus(f.vcpus()), Some(f), "{f:?} round-trip");
        }
    }
    ...
    #[test]
    fn from_vcpus_rejects_non_catalogue_counts() {
        // Counts no flavor uses (0, 3, 255) map to None so the preflight
        // gate skips the RAM half rather than mis-attributing a budget.
        for n in [0u8, 3, 5, 7, 33, 255] {
            assert_eq!(Flavor::from_vcpus(n), None, "vcpus={n} must be None");
        }
    }
}
