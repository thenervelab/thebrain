...
The total weight budget is `MAX_SCORE = 65,535`. This budget is split between miners and the burn UID (UID 0) based on a dynamically computed **burn percentage**.
...
```
uid_zero_pool = 65,535 * burn_percentage     (goes to the burn UID 0)
miners_pool   = 65,535 - uid_zero_pool       (distributed among miners)
```

**What this means for miners**: When the network stores more data (higher cost relative to emissions), the burn percentage decreases, giving miners a larger share of the weight pool. When emissions far exceed storage cost, more weight goes to the burn UID.
...
[`uid_zero_weight()`](pallets/execution-unit/src/weight_calculation.rs)

The burn UID receives the `uid_zero_pool` weight from Layer 3. This is capped so the total of all weights (miners + burn UID) does not exceed 65,535:
...
| **UID 0** | The burn UID in the Bittensor subnet; receives the burn-percentage portion of weights |
