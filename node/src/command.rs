...
impl SubstrateCli for Cli {
	...
	fn support_url() -> String {
		"https://github.com/thenervelab/thebrain/issues".into()
	}
	...
	fn load_spec(&self, id: &str) -> Result<Box<dyn sc_service::ChainSpec>, String> {
		Ok(match id {
			"" | "dev" | "local" => Box::new(chainspec::testnet::local_testnet_config(
				hippius_primitives::TESTNET_LOCAL_CHAIN_ID,
			)?),
			// generates the spec for benchmarking.
			"benchmark" => Box::new(chainspec::testnet::local_benchmarking_config(
				hippius_primitives::TESTNET_CHAIN_ID,
			)?),
			// generates the spec for testnet
			"testnet" => Box::new(chainspec::testnet::hippius_testnet_config(
				hippius_primitives::TESTNET_CHAIN_ID,
			)?),
			"hippius-testnet" => Box::new(chainspec::testnet::ChainSpec::from_json_bytes(
				&include_bytes!("../../chainspecs/testnet/hippius-testnet.json")[..],
			)?),
			...
			"mainnet" => Box::new(chainspec::mainnet::hippius_mainnet_config(
				hippius_primitives::MAINNET_CHAIN_ID,
			)?),
			...
			runner.sync_run(|config| {
				...
				match cmd {
					BenchmarkCmd::Pallet(cmd) => {
						...
						<crate::service::hippius::ExecutorDispatch as sc_executor::NativeExecutionDispatch>::ExtendHostFunctions>(Some(
							config.chain_spec,
						))
					},
