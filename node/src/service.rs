...
#[cfg(feature = "testnet")]
use hippius_testnet_runtime::{self, RuntimeApi, TransactionConverter};

// Assuming you have the necessary imports at the top of the file
// use pallet_execution_unit::Pallet as ExecutionUnitPallet; // Adjust the path as necessary
...
pub mod hippius {
	...
	impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
		...
		fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
			hippius_testnet_runtime::api::dispatch(method, data)
		}

		fn native_version() -> sc_executor::NativeVersion {
			hippius_testnet_runtime::native_version()
		}
	}
}
...
) -> Result<TaskManager, ServiceError>
{
	...
	if config.role.is_authority() {
		...
		if crate::utils::ensure_all_keys_exist_in_keystore(keystore_container.keystore()).is_err() {
			...
			++++++++++++++++++++++++++++++++++++++++++++++++
				...
				Hippius Network, refer to doc on
			...
			++++++++++++++++++++++++++++++++++++++++++++++++
				Validator keys not found, validator keys are essential to run a validator on
				Hippius Network, refer to the doc on
	...
	if config.offchain_worker.enabled {
		...
		// Store the PeerId in offchain storage
		let local_peer_id: PeerId = network.local_peer_id();
		let peer_id_bytes = local_peer_id.to_bytes().to_vec(); // Convert PeerId to Vec<u8>

		// Use the offchain_db to store a sample value
		let offchain_db = backend.offchain_storage();
		// Use the offchain_db to store the PeerId
		if let Some(mut db) = offchain_db {
			db.set(b"storage", b"node_id", &peer_id_bytes); // Store the PeerId
		} else {
			log::error!("Offchain database is not available.");
		}
		...
		if let Some(bandwidth_mbps) = bandwidth_mbps {
			...
			// Check if offchain_db is Some and then use it
			if let Some(mut db) = offchain_db {
				db.set(b"storage", b"bandwidth_mbps", &bandwidth_mbps_bytes);
			} else {
				log::error!("Offchain database is not available.");
			}
		}
		...
		if let Some(storage) = storage_bytes {
			...
			// Check if offchain_db is Some and then use it
			if let Some(mut db) = offchain_db {
				db.set(b"storage", b"storage_bytes", &storage_bytes);
			} else {
				log::error!("Offchain database is not available.");
			}
		}
		...
		if let Some(uptime) = uptime {
			...
			// Check if offchain_db is Some and then use it
			if let Some(mut db) = offchain_db {
				db.set(b"storage", b"uptime_bytes", &uptime_bytes);
			} else {
				log::error!("Offchain database is not available.");
			}
		}
		...
		if let Some(peers) = peers {
			// Convert `peers` (u16) to a byte representation
			let peers_bytes = peers.to_le_bytes();
			...
			// Check if offchain_db is Some and then use it
			if let Some(mut db) = offchain_db {
				db.set(b"storage", b"peers_bytes", &peers_bytes);
			} else {
				log::error!("Offchain database is not available.");
			}
		}
		...
		if let Some(latency) = latency {
			...
			// Check if offchain_db is Some and then use it
			if let Some(mut db) = offchain_db {
				db.set(b"storage", b"latency_bytes", &latency_bytes);
			} else {
				log::error!("Offchain database is not available.");
			}
		}
