...
pub mod pallet {
    ...
    use sp_runtime::{
        offchain::{Duration,http,storage_lock::{StorageLock, BlockAndTime}},
    ...
    #[pallet::config]
    pub trait Config: frame_system::Config + SendTransactionTypes<Call<Self>> +  pallet_babe::Config + pallet_session::Config + pallet_staking::Config
     +  pallet_registration::Config +  frame_system::offchain::SigningTypes{
    ...
    impl<T: Config> ValidateUnsigned for Pallet<T> {
        ...
        fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
            match call {
                Call::submit_hot_keys { hot_keys, signature: _ } => {
                    ValidTransaction::with_tag_prefix("MetagraphOffchain")
                    .priority(TransactionPriority::max_value())
                    .and_provides(("submit_hot_keys", hot_keys.len() as u64))
                    .longevity(5)
                    .propagate(true)
                    .build()
                }
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn check_consensus(submissions: &BTreeMap<T::AccountId, Vec<UID>>, new_submission: &Vec<UID>) -> bool {
            ...
            let avg_size: f64 = submissions.values()
                .map(|v| v.len() as f64)
                .sum::<f64>() / submissions.len() as f64;
            ...
            let size_divergence = ((if new_size > avg_size { 
                new_size - avg_size 
            } else { 
                avg_size - new_size 
            ...
            for (validator, existing_submission) in submissions.iter() {
                if existing_submission == new_submission {
                    ...
                    let common_uids: Vec<_> = existing_submission.iter()
                        ...
                        *points = points.saturating_sub(1);
                        ...
                        if *points <= -3 {
                            ...
                            if let Some(current_era) = <pallet_staking::Pallet<T>>::current_era() {
                                ...
                                if let Some(mut ledger) = <pallet_staking::Ledger<T>>::get(&validator) {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn submit_hot_keys(
            ...
            if Self::check_consensus(&submissions, &hot_keys) {
                ...
                Self::deposit_event(Event::StorageUpdated {
                    uids_count: hot_keys.len() as u32,
                });
            }
