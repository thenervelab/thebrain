use crate::{crypto::MetagraphAuthId, mock::*, pallet, Error, Event, Role, UID};
use frame_support::{assert_noop, assert_ok};
use frame_system::offchain::{SignedPayload, SigningTypes};
use sp_core::{
	crypto::{Pair as TraitPair, Ss58Codec},
	sr25519,
};
...
fn test_submit_hot_keys() {
	new_test_ext().execute_with(|| {
		...
		// Create test UIDs
		let uids = vec![UID { address: public.clone(), id: 19, role: Role::Validator }];

		// Create signed payload
		let payload =
			pallet::SignedPayloadType::<Test> { hot_keys: uids.clone(), public: public.clone() };
		...
		assert_noop!(
			MetagraphModule::submit_hot_keys(RuntimeOrigin::none(), payload, wrong_signature,),
...
fn test_unsigned_validation() {
	new_test_ext().execute_with(|| {
		...
		// Create signed payload
		let payload = pallet::SignedPayloadType::<Test> { hot_keys: uids, public: public.clone() };
		...
		let invalid_call =
			crate::Call::<Test>::submit_hot_keys { payload, signature: wrong_signature };

		assert!(
			MetagraphModule::validate_unsigned(TransactionSource::Local, &invalid_call).is_err()
		);
	});
}
...
fn test_ss58_encoding() {
	new_test_ext().execute_with(|| {
		...
		let uid = UID { address: public.clone(), id: 19, role: Role::None };
