...
pub mod pallet {
    ...
    pub type DataPublicKeys<T: Config> = StorageMap<
        _, 
        Blake2_128Concat, 
        T::AccountId, 
        Vec<u8>, 
    ...
    pub type MessagePublicKeys<T: Config> = StorageMap<
        _, 
        Blake2_128Concat, 
        T::AccountId, 
        Vec<u8>, 
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_username(origin: OriginFor<T>, username: Vec<u8>) -> DispatchResult {
            ...
            // Ensure the user does not already have a username set
                ...
                !AccountUsernames::<T>::contains_key(&who),
                Error::<T>::UsernameAlreadySet
            );
