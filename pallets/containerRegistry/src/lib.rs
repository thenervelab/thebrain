...
pub mod pallet {
    ...
    pub(super) type DigestInfoStorage<T: Config> = StorageMap<
        _,
        Blake2_128Concat, Vec<u8>,  // digest
    ...
    pub(super) type ImageDigestToCid<T: Config> = StorageDoubleMap<
        _,
        Blake2_128Concat, Vec<u8>,  // image_name
        Blake2_128Concat, Vec<u8>,  // digest
        Vec<u8>,                    // cid
        OptionQuery
    >;
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        fn on_initialize(_block_number: BlockNumberFor<T>) -> Weight {
            ...
            let cid_delete_requests = pallet_marketplace::Pallet::<T>::registry_cid_delete_requests();
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_space(origin: OriginFor<T>, name: Vec<u8>) -> DispatchResult {
            ...
            let space_id = space_id.wrapping_add(
                frame_system::Pallet::<T>::block_number().try_into().unwrap_or(0)
            );
            
            // Ensure space doesn't already exist
            ensure!(
                !Spaces::<T>::contains_key(space_id),
                Error::<T>::SpaceAlreadyExists
            );
        ...
        pub fn add_space_member(
            ...
            // Retrieve space
            let mut space = Spaces::<T>::get(space_id)
        ...
        pub fn add_manifest_head_digest_and_manifest_json_cid(
            ...
            Self::deposit_event(Event::ImageDigestToCidStored {
                who,
                image_name,
                digest,
                cid,
        ...
        pub fn store_digest_info(
            ...
            let file_input = FileInput {
                file_name: "Digest Info".as_bytes().to_vec(),
                file_hash: cid.clone(),
            };
