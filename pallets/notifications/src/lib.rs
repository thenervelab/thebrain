...
pub mod pallet {
    ...
    pub type LastCallTime<T: Config> = StorageMap<
        _,
        Blake2_128Concat,
        T::AccountId,
        BlockNumberFor<T>,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn send_notification(
            ...
            if let Some(last_call) = LastCallTime::<T>::get(&sender) {
                ensure!(current_block > last_call + T::CooldownPeriod::get().into(), Error::<T>::CooldownNotElapsed);
            }
            ...
            Self::deposit_event(Event::NotificationSent(
                sender,
                recipient,
                block_to_send,
            ));
    ...
    impl<T: Config> Pallet<T> {
        pub fn notify_subscription_ended(sender: T::AccountId, recipient: T::AccountId, subscription_id: u32) {
            let notification = Notification {
                sender: sender,
        ...
        pub fn notify_subscription_ending_soon(sender: T::AccountId, recipient: T::AccountId, subscription_id: u32) {
            let notification = Notification {
                sender: sender,
                block_to_send: <frame_system::Pallet<T>>::block_number(),
                recurrence: false,
