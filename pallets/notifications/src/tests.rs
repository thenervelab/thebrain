...
fn send_notification_works() {
	new_test_ext().execute_with(|| {
		...
		// Send a notification
		assert_ok!(Notifications::send_notification(
...
fn mark_notification_as_read() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Notifications::mark_as_read(RuntimeOrigin::signed(recipient), 0));
...
fn sudo_update_notification_works() {
	new_test_ext().execute_with(|| {
		...
		let new_block = 20;
		assert_ok!(Notifications::sudo_update_notification(
			RawOrigin::Root.into(),
			recipient,
			0,
			new_block,
...
fn ban_account_prevents_notifications() {
	new_test_ext().execute_with(|| {
		...
		// Ban the account
		assert_ok!(Notifications::ban_account(RawOrigin::Root.into(), banned_sender));
