...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn submit_compute_request_assignment(
			...
			ComputeRequests::<T>::iter().for_each(|(owner, requests)| {
				if let Some(request_index) =
					requests.iter().position(|r| r.request_id == request_id)
				{
			...
			let node_info =
				RegistrationPallet::<T>::get_node_registration_info(miner_node_id.clone());
		...
		pub fn mark_miner_compute_stop_request_fulfilled(
			...
			let request_found = user_stop_requests
				.iter_mut()
		...
		pub fn request_compute_stop(origin: OriginFor<T>, plan_id: T::Hash) -> DispatchResult {
			// Ensure the caller is signed
			let user = ensure_signed(origin)?;

			// Check if the account is a sub-account, and if so, use the main account
			let main_account =
				match <pallet_subaccount::Pallet<T> as SubAccounts<T::AccountId>>::get_main_account(
					user.clone(),
				) {
					Ok(main) => main,
		...
		#[pallet::weight((10_000, DispatchClass::Normal, Pays::Yes))]
		pub fn request_compute_boot(origin: OriginFor<T>, plan_id: T::Hash) -> DispatchResult {
			// Ensure the caller is signed
			let user = ensure_signed(origin)?;
		...
		#[pallet::weight((10_000, DispatchClass::Normal, Pays::Yes))]
		pub fn request_compute_delete(origin: OriginFor<T>, plan_id: T::Hash) -> DispatchResult {
			// Ensure the caller is signed
			let user = ensure_signed(origin)?;
		...
		#[pallet::weight((10_000, DispatchClass::Normal, Pays::Yes))]
		pub fn request_compute_reboot(origin: OriginFor<T>, plan_id: T::Hash) -> DispatchResult {
			// Ensure the caller is signed
			let user = ensure_signed(origin)?;
		...
		pub fn request_compute_resize(
			...
			// Call the compute pallet's stop request function
			Self::add_miner_compute_image_resize_request(
				main_account.clone(),
				plan_id,
				resize_to_gbs,
			)?;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_compute_request(
			...
			Self::deposit_event(Event::ComputeRequestCreated { plan_id, owner });

			Ok(())
		}

		/// Helper method to get all pending compute requests
		pub fn get_pending_compute_requests(
		) -> Vec<ComputeRequest<T::AccountId, BlockNumberFor<T>, T::Hash>> {
		...
		pub fn get_unfulfilled_miner_compute_requests(
			node_id: Vec<u8>,
		) -> Vec<MinerComputeRequest<BlockNumberFor<T>, T::Hash, T::AccountId>> {
			MinerComputeRequests::<T>::get(&node_id)
				.into_iter()
				.filter(|request| {
					!request.fullfilled && request.job_id.is_none() && request.fail_reason.is_none()
				})
				.collect()
		}

		/// Helper function to get miner compute requests with a job_id that are not yet fulfilled
		pub fn get_pending_job_requests(
			node_id: Vec<u8>,
		) -> Vec<MinerComputeRequest<BlockNumberFor<T>, T::Hash, T::AccountId>> {
			MinerComputeRequests::<T>::get(&node_id)
				.into_iter()
				.filter(|request| {
					request.job_id.is_some() && !request.fullfilled && request.fail_reason.is_none()
				})
				.collect()
		}

		/// Helper function to get miner compute requests with a job_id that are not yet fulfilled
		pub fn get_pending_vnc_requests(
			node_id: Vec<u8>,
		) -> Vec<MinerComputeRequest<BlockNumberFor<T>, T::Hash, T::AccountId>> {
			MinerComputeRequests::<T>::get(&node_id)
				.into_iter()
				.filter(|request| {
					request.job_id.is_some()
						&& request.fail_reason.is_none()
						&& request.vnc_port.is_none()
				})
				.collect()
		}

		/// Helper function to get miner compute requests with a job_id that are not yet fulfilled
		pub fn get_pending_nebula_requests(
			node_id: Vec<u8>,
		) -> Vec<MinerComputeRequest<BlockNumberFor<T>, T::Hash, T::AccountId>> {
			MinerComputeRequests::<T>::get(&node_id)
				.into_iter()
		...
		pub fn get_miner_compute_request(
			...
			for (_, miner_requests) in MinerComputeRequests::<T>::iter() {
				...
				if let Some(miner_request) =
					miner_requests.iter().find(|req| req.plan_id == plan_id && req.fullfilled)
				{
					// Check the corresponding compute request to match the account ID
					let compute_request_match =
						ComputeRequests::<T>::get(&account_id).iter().any(|req| {
							req.plan_id == plan_id && req.request_id == miner_request.request_id
						});
		...
		pub fn get_miner_compute_request_by_id(
			...
			for (_, miner_requests) in MinerComputeRequests::<T>::iter() {
				...
				if let Some(miner_request) =
					miner_requests.iter().find(|req| req.request_id == request_id && req.fullfilled)
				{
					return Some(miner_request.clone());
				}
			}
		...
		fn find_miner_compute_request(
			...
			// Reuse  method
			Self::get_miner_compute_request(account_id, plan_id).map(|miner_request| {
				(miner_request.request_id, miner_request.miner_node_id, miner_request.job_id)
			})
		}
		...
		pub fn add_delete_miner_compute_request(
			...
			let (request_id, node_id, job_id) =
				Self::find_miner_compute_request(user_id.clone(), plan_id.clone())
			...
			MinerComputeStopRequests::<T>::mutate(&node_id, |stop_requests| {
				...
				stop_requests.retain(|req| !(req.user_id == user_id && req.plan_id == plan_id));
			});
			...
			MinerComputeBootRequests::<T>::mutate(&node_id, |boot_requests| {
				...
				boot_requests.retain(|req| !(req.user_id == user_id && req.plan_id == plan_id));
			});
			...
			MinerComputeRebootRequests::<T>::mutate(&node_id, |reboot_requests| {
				...
				reboot_requests.retain(|req| !(req.user_id == user_id && req.plan_id == plan_id));
			});

			// Check and delete any matching reboot requests
			MinerComputeResizeRequests::<T>::mutate(&node_id, |resize_requests| {
				// Remove reboot requests that match the user_id, plan_id, and are not fulfilled
				resize_requests.retain(|req| !(req.user_id == user_id && req.plan_id == plan_id));
			});
			...
			// Emit event
			Self::deposit_event(Event::ComputeRequestDeleted {
				request_id,
				owner: user_id.clone(),
			});
		...
		pub fn call_handle_miner_compute_request_failure(
			...
			if let Ok(_guard) = lock.try_lock() {
				...
				for (acc, res) in &results {
					match res {
						Ok(_) => log::info!(
							"[{:?}] Successfully marked compute request as fulfilled",
							acc.id
						),
						Err(e) => log::info!(
							"[{:?}] Failed to mark compute request as fulfilled: {:?}",
							acc.id,
							e
						),
					}
		...
		pub fn mark_compute_request_fulfilled_offchain(node_id: Vec<u8>, request_id: u128) {
			let mut lock =
				StorageLock::<BlockAndTime<frame_system::Pallet<T>>>::with_block_and_time_deadline(
			...
			if let Ok(_guard) = lock.try_lock() {
				...
				for (acc, res) in &results {
					match res {
						...
						Err(e) => {
							log::info!("[{:?}] Failed to submit compute update: {:?}", acc.id, e)
						},
					}
				}
			} else {
		...
		pub fn call_submit_compute_boot_request_fulfillment(node_id: Vec<u8>, request_id: u128) {
			let mut lock =
				StorageLock::<BlockAndTime<frame_system::Pallet<T>>>::with_block_and_time_deadline(
			...
			if let Ok(_guard) = lock.try_lock() {
				...
				for (acc, res) in &results {
					match res {
						Ok(_) => log::info!(
							"[{:?}] Successfully marked compute request as deleted",
							acc.id
						),
						Err(e) => log::info!(
							"[{:?}] Failed to mark boot request as deleted: {:?}",
							acc.id,
							e
						),
					}
		...
		pub fn call_submit_compute_reboot_request_fulfillment(node_id: Vec<u8>, request_id: u128) {
			let mut lock =
				StorageLock::<BlockAndTime<frame_system::Pallet<T>>>::with_block_and_time_deadline(
			...
			if let Ok(_guard) = lock.try_lock() {
				...
				for (acc, res) in &results {
					match res {
						Ok(_) => log::info!(
							"[{:?}] Successfully marked reboot request as deleted",
							acc.id
						),
						Err(e) => log::info!(
							"[{:?}] Failed to mark reboot request as deleted: {:?}",
							acc.id,
							e
						),
					}
		...
		//
		pub fn call_submit_compute_resize_request_fulfillment(node_id: Vec<u8>, request_id: u128) {
			let mut lock =
				StorageLock::<BlockAndTime<frame_system::Pallet<T>>>::with_block_and_time_deadline(
			...
			if let Ok(_guard) = lock.try_lock() {
				...
				for (acc, res) in &results {
					match res {
						Ok(_) => log::info!(
							"[{:?}] Successfully marked resize request as fulfilled",
							acc.id
						),
						Err(e) => log::info!(
							"[{:?}] Failed to mark resize request as fulfilled: {:?}",
							acc.id,
							e
						),
					}
		...
		pub fn call_submit_compute_deletion_request(
			...
			if let Ok(_guard) = lock.try_lock() {
				...
				for (acc, res) in &results {
					match res {
						Ok(_) => log::info!(
							"[{:?}] Successfully marked delete compute request as fulfilled",
							acc.id
						),
						Err(e) => log::info!(
							"[{:?}] Failed to mark delete compute request as fulfilled: {:?}",
							acc.id,
							e
						),
					}
		...
		pub fn delete_miner_compute_stop_request(
			...
			let request_index = stop_requests
				.iter()
				.position(|req| req.user_id == *user_id && req.plan_id == *plan_id)
		...
		fn handle_request_assignment(
			...
			for miner_request in miner_requests {
				let compute_request =
					ComputeRequests::<T>::iter().find_map(|(_owner, requests)| {
						requests
							.iter()
				...
				if let Some(ip) = vm_available_ips.pop() {
					if let Some(compute_request) = compute_request {
						...
						match serde_json::from_slice::<TechnicalDescription>(
							...
							Ok(tech_desc) => {
								...
								let deadline = sp_io::offchain::timestamp()
									.add(Duration::from_millis(100_000));
								...
								match serde_json::from_str::<serde_json::Value>(response_str) {
									Ok(json_response) => {
										if let (Some(job_id), Some(_name), Some(_status)) = (
											...
											log::warn!(
												"Missing expected fields in VM creation response"
											);
										}
									},
									Err(e) => {
										log::error!(
											"Failed to parse VM creation response JSON: {:?}",
											e
										);
										return Err(sp_runtime::offchain::http::Error::Unknown);
									},
		...
		// stop the vm
		fn handle_stop_request_assignment(
			node_id: Vec<u8>,
		) -> Result<(), sp_runtime::offchain::http::Error> {
			...
			for miner_request in miner_requests {
				...
				if !miner_request.fullfilled {
					...
					if let Some(compute_request) =
						Self::find_compute_request_by_id(miner_request.request_id)
					{
						if compute_request.1.status == ComputeRequestStatus::Running {
							// Use the new generic function to stop VM
							Self::perform_vm_operation(
								"stop",
		...
		// boot the vm
		fn handle_boot_request_assignment(
			node_id: Vec<u8>,
		) -> Result<(), sp_runtime::offchain::http::Error> {
		...
		// reboot the vm
		fn handle_reboot_request_assignment(
			node_id: Vec<u8>,
		) -> Result<(), sp_runtime::offchain::http::Error> {
		...
		fn handle_pending_job_requests(
			node_id: Vec<u8>,
		) -> Result<(), sp_runtime::offchain::http::Error> {
					...
					Ok(json_response) => {
						if let (Some(job_id), Some(name), Some(status)) = (
							...
							match status {
								"Completed" => {
									// Mark the request as fulfilled
									Self::mark_compute_request_fulfilled_offchain(
										node_id.clone(),
										miner_request.request_id,
									);
								},
								...
								"Failed" => {
									...
									Self::call_handle_miner_compute_request_failure(
										node_id.clone(),
										miner_request.request_id,
		...
		fn handle_delete_request_assignment(
			node_id: Vec<u8>,
		) -> Result<(), sp_runtime::offchain::http::Error> {
			...
			for miner_request in miner_requests {
				...
				let vm_name =
					format!("vm-{}", String::from_utf8_lossy(&job_id).split('-').next().unwrap());
		...
		pub fn add_miner_compute_stop_request(
			...
			// Attempt to find the request details and minner node id
			let (request_id, miner_node_id, job_id) =
				Self::find_miner_compute_request(user_id.clone(), plan_id.clone())
					.ok_or(Error::<T>::ComputeRequestNotFound)?;
			...
			let request_exists = user_stop_requests
				.iter()
			...
			if !request_exists {
				...
				log::info!("Compute stop request for plan {:?} already exists", plan_id);
			}
		...
		pub fn add_miner_compute_boot_request(
			...
			// Check if a request with the same plan_id already exists
			let request_exists = user_boot_requests
				.iter()
				.any(|req| req.plan_id == plan_id && req.user_id == user_id && !req.fullfilled);
			...
			if !request_exists {
				...
				log::info!("Compute boot request for plan {:?} already exists", plan_id);
			}
		...
		pub fn add_miner_compute_image_resize_request(
			...
			// Check if a request with the same plan_id already exists
			let request_exists = user_resize_requests
				.iter()
				.any(|req| req.plan_id == plan_id && req.user_id == user_id && !req.fullfilled);
			...
			if !request_exists {
				...
				log::info!("Compute resize request for plan {:?} already exists", plan_id);
			}
		...
		pub fn get_compute_request_by_id(
			request_id: u128,
		) -> Option<ComputeRequest<T::AccountId, BlockNumberFor<T>, T::Hash>> {
			ComputeRequests::<T>::iter().find_map(|(_owner, requests)| {
				requests.iter().find(|req| req.request_id == request_id).cloned()
			})
		}
		...
		pub fn delete_miner_compute_boot_request(
			...
			let request_index = boot_requests
				.iter()
				.position(|req| {
					req.user_id == *user_id && req.plan_id == *plan_id && !req.fullfilled
				})
				.ok_or(Error::<T>::ComputeBootRequestNotFound)?;
		...
		pub fn compute_boot_request_exists(user_id: &T::AccountId, plan_id: &T::Hash) -> bool {
			// Attempt to find the miner account id for the compute request
			let miner_node_id =
				match Self::find_miner_compute_request(user_id.clone(), plan_id.clone()) {
			...
			user_boot_requests.iter().any(|req| req.plan_id == *plan_id && !req.fullfilled)
		}
		...
		pub fn compute_stop_request_exists(user_id: &T::AccountId, plan_id: &T::Hash) -> bool {
		...
		fn get_current_timestamp() -> u64 {
			...
			block_number.wrapping_mul(1000) + (block_number % 1000) + (block_number & 0xFF)
		}
		...
		pub fn find_compute_request_by_id(
			...
			ComputeRequests::<T>::iter().find_map(|(owner, requests)| {
				requests
					.iter()
		...
		pub fn add_miner_compute_reboot_request(
			...
			// Check if a request with the same plan_id already exists
			let request_exists = user_reboot_requests
				.iter()
				.any(|req| req.plan_id == plan_id && req.user_id == user_id && !req.fullfilled);
			...
			if !request_exists {
				...
				log::info!("Compute reboot request for plan {:?} already exists", plan_id);
			}
			Ok(())
		}
						...
						Some(node_info) => {
							if node_info.node_type == NodeType::ComputeMiner {
								match Self::handle_request_assignment(node_info.node_id.clone()) {
									...
									Err(e) => {
										log::error!("Error in request assignment handling: {:?}", e)
									},
								}

								match Self::handle_pending_job_requests(node_info.node_id.clone()) {
									Ok(_) => {
										log::info!("Pending job requests handled successfully")
									},
									Err(e) => log::error!(
										"Error in pending job request handling: {:?}",
										e
									),
								}
								...
								match Self::handle_pending_vnc_requests(node_info.node_id.clone()) {
									Ok(_) => {
										log::info!("Pending vnc requests handled successfully")
									},
									Err(e) => log::error!(
										"Error in pending vnc request handling: {:?}",
										e
									),
								}
								...
								match Self::handle_delete_request_assignment(
									...
									Err(e) => {
										log::error!("Error in delete request handling: {:?}", e)
									},
								}

								// handle stop requests of minners
								match Self::handle_stop_request_assignment(
									node_info.node_id.clone(),
								) {
									Ok(_) => log::info!("Stop Request handled successfully"),
									Err(e) => {
										log::error!("Error in stop request handling: {:?}", e)
									},
								}

								// handle boot requests of minners
								match Self::handle_boot_request_assignment(
									node_info.node_id.clone(),
								) {
									Ok(_) => log::info!("boot Request handled successfully"),
									Err(e) => {
										log::error!("Error in boot request handling: {:?}", e)
									},
								}

								// handle reboot requests of minners
								match Self::handle_reboot_request_assignment(
									node_info.node_id.clone(),
								) {
