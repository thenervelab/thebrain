...
#[test]
fn test_reward_distribution_logic() {
	println!("\n=== Testing Staking Reward Distribution Logic ===\n");

	// Test the core distribution logic without needing full runtime setup
	let total_reward: u128 = 1_000_000;
	let validator_exposure: u128 = 50_000_000; // 50M
	let nominator_exposure: u128 = 30_000_000; // 30M
	let total_exposure = validator_exposure + nominator_exposure; // 80M

	// Calculate proportional rewards
	let validator_reward = (total_reward * validator_exposure) / total_exposure;
	let nominator_reward = (total_reward * nominator_exposure) / total_exposure;

	println!("Setup:");
	println!("  Total reward: {}", total_reward);
	println!("  Validator exposure: {} ({:.1}%)", validator_exposure, (validator_exposure as f64 / total_exposure as f64) * 100.0);
	println!("  Nominator exposure: {} ({:.1}%)", nominator_exposure, (nominator_exposure as f64 / total_exposure as f64) * 100.0);

	println!("\nCalculated rewards:");
	println!("  Validator: {} ({:.1}%)", validator_reward, (validator_reward as f64 / total_reward as f64) * 100.0);
	println!("  Nominator: {} ({:.1}%)", nominator_reward, (nominator_reward as f64 / total_reward as f64) * 100.0);
	println!("  Total: {}", validator_reward + nominator_reward);

	// Verify the math
	assert_eq!(validator_reward + nominator_reward, total_reward, "Total should match");
	assert!(validator_reward > nominator_reward, "Validator should earn more (larger stake)");
	assert_eq!(
		validator_reward, 625_000,
		"Validator should get 62.5% (50/80 * 1M) = 625k"
	);
	assert_eq!(
		nominator_reward, 375_000,
		"Nominator should get 37.5% (30/80 * 1M) = 375k"
	);

	println!("\n✅ Test passed! Reward distribution logic is correct:");
	println!("   Validator: {} (62.5%)", validator_reward);
	println!("   Nominator: {} (37.5%)", nominator_reward);
