...
impl NodeMetricsData {
	...
	fn pool_context(
		...
		let cost = total_resource_gb * price_per_gb_token * 100.0;
		info!("cost: {}", cost);
	...
	>(
		...
		let node_info =
			pallet_registration::Pallet::<T>::get_node_registration_info(miner_id.clone());
		...
		let final_weight = Self::calculate_final_weight(
			arion_weight as u32,
			total_arion_weight as u32,
			miners_pool_inner as u32,
		);
		final_weight
	}
