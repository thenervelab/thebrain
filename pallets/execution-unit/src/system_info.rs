use crate::types::{
	DiskDetails, DiskInfo, NetworkDetails, NetworkInterfaceInfo, NetworkType, SystemInfo,
...
impl SystemInfo {
	...
	pub fn get_ipfs_repo_stat() -> Result<(u64, u64), http::Error> {
		...
		let repo_size = extract_number(body_str, "\"RepoSize\"").ok_or_else(|| {
			log::warn!("Failed to parse RepoSize");
			http::Error::Unknown
		})?;

		let storage_max = extract_number(body_str, "\"StorageMax\"").ok_or_else(|| {
...
impl FromStr for SystemInfo {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		let storage_total_mb = if let Some(start) = s.find("\"storage_total_mb\":") {
			...
			if let Some(end) = after_key.find(",") {
				after_key[..end]
					.trim()
					.parse()
					.map_err(|_| "Failed to parse storage_total_mb")?
			} else {
		...
		let storage_free_mb = if let Some(start) = s.find("\"storage_free_mb\":") {
			...
			if let Some(end) = after_key.find(",") {
				after_key[..end]
					.trim()
					.parse::<u64>()
					.map_err(|_| "Failed to parse storage_free_mb")?
			} else {
		...
		let network_bandwidth_mb_s = if let Some(start) = s.find("\"network_bandwidth_mb_s\":") {
			...
			if let Some(end) = after_key.find(",") {
				after_key[..end]
					.trim()
					.parse()
					.map_err(|_| "Failed to parse network_bandwidth_mb_s")?
			} else {
		...
		if let Some(start) = s.find("\"uplink_mb\":") {
			...
			if let Some(end) = after_key.find(",") {
				interface.uplink_mb =
					after_key[..end].trim().parse().map_err(|_| "Failed to parse uplink_mb")?;
			}
		}
		...
		if let Some(start) = s.find("\"downlink_mb\":") {
			...
			if let Some(end) = after_key.find("},") {
				interface.downlink_mb =
					after_key[..end].trim().parse().map_err(|_| "Failed to parse downlink_mb")?;
			}
		}
		...
		fn parse_single_disk_details(disk_str: &str) -> Result<DiskDetails, &'static str> {
			...
			if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(disk_str) {
				if let Some(obj) = json_value.as_object() {
					...
					is_rotational =
						obj.get("is_rotational").and_then(|v| v.as_bool()).unwrap_or(false);
			...
			Ok(DiskDetails { name, serial, model, size, is_rotational, disk_type })
		}
				...
				if let Some(after_end) = s[start + after_start..].find("]") {
					...
					for disk in disks_str.split("},{").filter(|d| !d.is_empty()) {
						...
						if let Some(start) = cleaned_disk_str.find("\"total_space_mb\":") {
							let after_key =
								&cleaned_disk_str[start + "\"total_space_mb\":".len()..];
							if let Some(end) = after_key.find(",") {
								total_space_mb = after_key[..end]
									.trim()
									.parse()
									.map_err(|_| "Failed to parse total_space_mb")?;
							} else if let Some(end) = after_key.find('"') {
								total_space_mb = after_key[..end]
									.trim()
									.parse()
						...
						if let Some(start) = cleaned_disk_str.find("\"free_space_mb\":") {
							...
							if let Some(end) = after_key.find(",") {
								free_space_mb = after_key[..end]
									.trim()
									.parse()
									.map_err(|_| "Failed to parse free_space_mb")?;
							} else {
								free_space_mb = after_key
									.trim()
									.parse()
									.map_err(|_| "Failed to parse free_space_mb")?;
							}
						...
						disks.push(DiskInfo { name, disk_type, total_space_mb, free_space_mb });
					}
				}
			}
		}
		...
		if let Some(start) = s.find("\"zfs_info\":[") {
			...
			if let Some(end) = after_start.find("]") {
				...
				for zfs_item in zfs_str.split(',') {
					let cleaned_zfs_item = zfs_item.trim().trim_matches('"').as_bytes().to_vec();
		...
		if let Some(start) = s.find("\"raid_info\":[") {
			...
			if let Some(end) = after_start.find("]") {
				...
				for raid_item in raid_str.split(',') {
					let cleaned_raid_item = raid_item.trim().trim_matches('"').as_bytes().to_vec();
