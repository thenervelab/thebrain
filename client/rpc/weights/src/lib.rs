...
pub fn create_signed_extrinsic(netuid: u16, dests: Vec<u16>, weights: Vec<u16>, version_key: u64, keystore: KeystorePtr) -> Vec<u8> {
    let data = (netuid, dests, weights, version_key);
    ...
    // Define the spec_version, transaction_version, tip, and mode
    let spec_version: u32 = 212; // Replace with actual spec version
    ...
    let gensis_hash_result = hex_to_array("0x8fa3c25746c6e18aaee4b23e906fdae672d9f8049f6a8e7fe2e4650b5750d78b");
    let gensis_hash = gensis_hash_result.unwrap();
    ...
    let nonce: u64 = match fetch_nonce(&public_key_bytes) {
        Ok(value) => value,
        Err(_e) => {
            27
        }
...
fn fetch_nonce(account: &[u8; 32]) -> Result<u64, &'static str> {
    let remote_url = "https://tao-testnet.hippius.network";
            ...
            match obj.into_iter().find(|(k, _)| k.iter().copied().eq("result".chars())) {
                Some((_, value)) => {
                    ...
                    match value {
                        ...
                        _ => {
                            log::error!("Result field is not a number, got unexpected JSON value type");
