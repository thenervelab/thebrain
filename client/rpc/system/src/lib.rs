...
pub use rpc_core_system::{
	types::{DiskDetails, DiskInfo, NetworkInterfaceInfo, SystemInfo},
	SystemInfoApiServer,
};
use sp_api::ProvideRuntimeApi;
use sp_blockchain::HeaderBackend;
use sp_runtime::traits::Block as BlockT;
use std::{collections::HashSet, fs, process::Command, process::Stdio, sync::Arc};
...
fn count_kvm_vms() -> Option<usize> {
		...
		.output()
	{
		if output.status.success() {
			...
			let vm_count = stdout.lines().filter(|line| !line.trim().is_empty()).count();
...
fn get_hypervisor_disk_type() -> Option<String> {
	...
	if let Ok(entries) = fs::read_dir("/sys/block") {
		for entry in entries.filter_map(Result::ok) {
			let path = entry.path();
			let device_name = path.file_name().and_then(|name| name.to_str()).unwrap_or("");
	...
	if let Ok(output) = Command::new("lsblk").arg("-d").arg("-o").arg("NAME,TYPE").output() {
		let stdout = String::from_utf8_lossy(&output.stdout);
...
fn get_vm_pool_disk_type() -> Option<String> {
	...
	if let Ok(output) = Command::new("virsh").arg("pool-list").arg("--all").output() {
		let stdout = String::from_utf8_lossy(&output.stdout);
		...
		for line in stdout.lines() {
			if line.contains("active") {
				...
				if let Ok(pool_path_output) =
					Command::new("virsh").arg("pool-dumpxml").arg(pool_name).output()
				{
					...
					if let Some(path_start) = pool_path_xml.find("<path>") {
						...
						if let Ok(lsblk_output) =
							Command::new("lsblk").arg("-no").arg("NAME,TYPE,TRAN").output()
						{
						...
						if let Ok(df_output) = Command::new("df").arg("-T").arg(pool_path).output()
						{
	...
	if let Ok(output) = Command::new("virsh").arg("pool-info").arg("default").output() {
		let stdout = String::from_utf8_lossy(&output.stdout);
...
impl<B: BlockT, C> SystemInfoImpl<B, C> {
	pub fn new(_client: Arc<C>) -> Self {
		Self { _client, _phantom_data: Default::default() }
	}
}
	...
	C: HeaderBackend<B> + Send + Sync,
{
	fn get_system_info(&self) -> RpcResult<SystemInfo> {
		...
		let disks_info: Vec<DiskInfo> = disks
			...
			.filter_map(|disk| {
				...
				if seen_disks.insert(name.clone()) {
					// Insert the name into the HashSet
					let disk_type = disk.file_system().to_string_lossy().to_string(); // Convert OsStr to String directly
					...
					Some(DiskInfo { name, disk_type, total_space_mb, free_space_mb }) // Return Some(DiskInfo) if it's a new disk
				} else {
		...
		let network_bandwidth_mb_s: u32 = networks
			.list()
			.iter()
			.map(|(_, data)| {
		...
		for (name, data) in networks.list().iter() {
			...
			let mac_address = networks
				.list()
				.iter()
				.filter_map(|(_, network)| Some(network.mac_address())) // if MAC is available
		...
		let (ipfs_zfs_pool_size, ipfs_zfs_pool_alloc, ipfs_zfs_pool_free) =
			match get_ipfs_pool_info() {
				Ok(values) => values,
...
fn get_raid_info() -> Vec<String> {
	...
	let lspci_raid = match Command::new("lspci").arg("-v").arg("-s").arg("*:*:*").output() {
		Ok(output) => {
			let output_str = String::from_utf8_lossy(&output.stdout);
			output_str
				.lines()
	...
	if lspci_raid.is_empty() {
		match Command::new("mdadm").arg("--detail").arg("--scan").output() {
			Ok(output) => {
				let output_str = String::from_utf8_lossy(&output.stdout);
...
fn get_ipfs_pool_info() -> Result<(u128, u128, u128), String> {
	match Command::new("zpool").arg("list").arg("ipfs").output() {
		Ok(output) => {
			let stdout = String::from_utf8_lossy(&output.stdout);
...
fn parse_size(size_str: &str) -> Result<u128, String> {
	...
	let (number_part, unit) = size_str
		.split_at(size_str.chars().position(|c| c.is_alphabetic()).unwrap_or(size_str.len()));

	let mut size = number_part
		.trim()
		.parse::<f64>()
		.map_err(|_| format!("Failed to parse number: {}", number_part))?;
